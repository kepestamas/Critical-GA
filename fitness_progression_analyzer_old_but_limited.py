...
def construct_generation_filename(run_id, prefix,input_file, budget, payoff_function):
    ...
    # timing<prefix>_<run_id>_<input_clean>_ke_*_kn_*_wb_<budget>_<payoff_function>
    return f"timing{prefix}_{run_id}_{input_clean}_ke_*_kn_*_wb_{budget}_{payoff_function}"
...
def main():
    ...
    with open(csv_file, 'r', newline='') as f:
        ...
        for row in reader:
            # Extract run_id number from format like "tune_1" or "run_1"
            run_id_str = row['run_id']
            if (run_id_str.startswith('tune_') or run_id_str.startswith('run_')) and '_' in run_id_str:
                prefix = run_id_str.split('_')[0]
            ...
            gen_pattern = construct_generation_filename(run_id, prefix, input_file, budget, payoff_function)
